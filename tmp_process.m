...
function stat_struct = tmp_process(celltype)
    ...
    for i = 1:length(fnames)
        ...
        %%% TMP TMP TMP
        %spike_raster( data_struct, currents{i});
...
%         attend_struct = attIn_attOut_Contrasts( data_struct, currents{i} );
        for j = 1:length(attend_struct)
            ...
            %plot_att_sdens_Modified(attend_struct(j));%, fnames(i), num2str(attend_struct(j).current));
...
function ffps = get_ffps( mfs, celltype )
    ...
    for i = 1:length(mfs.session)
        ...
        for j = 1:length( mfs.session(i).processed_files )
            ...
            if mfs.session(i).type(j) == celltype
