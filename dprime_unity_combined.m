function dprime_unity_combined( stat_struct, selected_current )
            ...
            if length(stat_struct(i).attend) > 1
                ...
                for j = 1:length(stat_struct(i).attend)
                    if (stat_struct(i).attend(j).current == selected_current)
                        ...
                        [dprime_subset vis_subset = get_maxAttIn_dprime( stat_struct(i).attend(j) );
...
% in' d' condition
function [dprime_subset, vis] = get_maxAttIn_dprime( attend )
